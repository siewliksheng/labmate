...
> since no live LLM was available in the session that generated it — see
> `labmate.experiment.generate_report()` for what actually produces this
> shape of document end-to-end once a model backend is configured.

---

# Lab Experiment Report

**Experiment:** DNA extraction from E. coli K-12 culture using a standard alkaline lysis / ethanol precipitation protocol, followed by formaldehyde-fixed gel electrophoresis imaging
**Status:** Reported | **Signed off by:** dr. lin (PI)
...
| Item | Status | Notes |
|---|---|---|
| Formaldehyde (gel fixation) | Resolved | H302 (harmful if swallowed), H314 (severe skin burns/eye damage) -- fume hood + goggles required |
| *E. coli* K-12 culture | Resolved | BSL-1 -- standard microbiological practices, non-pathogenic lab strain |
| Ethanol (precipitation step) | **Unresolved, acknowledged** | No matching entry in the local biosafety/SDS lookup -- signed off by dr. lin (PI) with the gap explicitly acknowledged, not silently cleared |
...
*The following is a suggestion for the researcher/PI to confirm, not an instruction:*
...
- Since the ethanol hazard lookup came back unresolved, consider adding ethanol to the lab's local SDS reference table so future prelab checks for this protocol resolve automatically.
